...
# Add production (in thousands of 60kg bags)
coffee_data$production <- case_when(
  coffee_data$country == "Brazil" ~ sample(50000:60000, nrow(coffee_data[coffee_data$country == "Brazil", ]), replace = TRUE),
  coffee_data$country == "Vietnam" ~ sample(25000:30000, nrow(coffee_data[coffee_data$country == "Vietnam", ]), replace = TRUE),
  coffee_data$country == "Colombia" ~ sample(12000:15000, nrow(coffee_data[coffee_data$country == "Colombia", ]), replace = TRUE),
  coffee_data$country == "Indonesia" ~ sample(10000:12000, nrow(coffee_data[coffee_data$country == "Indonesia", ]), replace = TRUE),
  coffee_data$country == "Ethiopia" ~ sample(6000:8000, nrow(coffee_data[coffee_data$country == "Ethiopia", ]), replace = TRUE),
  TRUE ~ sample(3000:7000, nrow(coffee_data[!(coffee_data$country %in% c("Brazil", "Vietnam", "Colombia", "Indonesia", "Ethiopia")), ]), replace = TRUE)
)

# Add exports (in thousands of 60kg bags)
coffee_data$exports <- coffee_data$production * runif(nrow(coffee_data), 0.5, 0.9)

# Add domestic consumption (in thousands of 60kg bags)
coffee_data$domestic_consumption <- coffee_data$production - coffee_data$exports

# Add price (USD per pound)
coffee_data$price <- case_when(
  coffee_data$country %in% c("Colombia", "Costa Rica", "Guatemala", "Kenya") ~ runif(nrow(coffee_data[coffee_data$country %in% c("Colombia", "Costa Rica", "Guatemala", "Kenya"), ]), 2.5, 4.5),
  TRUE ~ runif(nrow(coffee_data[!(coffee_data$country %in% c("Colombia", "Costa Rica", "Guatemala", "Kenya")), ]), 1.5, 3.5)
)

# Add variety
coffee_data$variety <- case_when(
  coffee_data$country %in% c("Brazil", "Vietnam", "India", "Indonesia") ~ "Robusta",
  TRUE ~ "Arabica"
)

# Add growing altitude (meters)
coffee_data$altitude <- case_when(
  coffee_data$country %in% c("Ethiopia", "Kenya", "Colombia", "Guatemala", "Costa Rica") ~ sample(1200:2000, nrow(coffee_data[coffee_data$country %in% c("Ethiopia", "Kenya", "Colombia", "Guatemala", "Costa Rica"), ]), replace = TRUE),
  TRUE ~ sample(600:1200, nrow(coffee_data[!(coffee_data$country %in% c("Ethiopia", "Kenya", "Colombia", "Guatemala", "Costa Rica")), ]), replace = TRUE)
)
```
